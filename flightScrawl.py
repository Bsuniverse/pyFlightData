...
class FlightSpider:
    ...
    def get_uids(self, page_number, token, cookie):
        ...
        res = self._session.post(url, data=json.dumps(postData), headers=header)
    ...
    def download_load_by_uid8(self, uid_8, token, cookie, path):
        ...
        with self._session.get(url=url, headers=header) as respond:
            ...
            try:
                ...
                print(f'Response is empty or data is not valid JSON, data uid8 is {uid_8}.')
    ...
    def download_by_uid8(self, uid_8, token, cookie, path):
        ...
        with open(path, "wb") as f:
            with self._session.post(url, data=json.dumps(postData), headers=header, stream=True) as r:
                r.raise_for_status()
...
def get_element(driver, pattern):
    ...
    '''
    try:
        element = WebDriverWait(driver, 10, 0.5).until(EC.presence_of_element_located(pattern))
    except TimeoutException:
        print("Timeout loading page, exit scrawling")
        return False
    else:
        return element
...
def get_ul_list_number(chrome) -> int:
    ul = chrome.find_element(by=By.XPATH, value='//*[@id="app"]/div/div[2]/section/div/div[2]/div[4]/div[1]/div[2]/ul')
...
def chrome_runner(user_name="SJZX_201528", user_password="dTMfHR3P9a!"):
    ...
    # 输入账号和密码
    element_account = get_element(chrome, (By.XPATH, '//*[@id="Calc"]/div[2]/div[1]/div[1]/div/div[1]/input'))
    ...
    ActionChains(chrome).click(element_passwd).send_keys_to_element(element_passwd, user_password).perform()
    # 勾选登录框并进行登录
    chrome.find_element(by=By.XPATH, value='//*[@id="Calc"]/div[4]/span[1]/div[1]').click()
    chrome.find_element(by=By.XPATH, value='//*[@id="Calc"]/div[5]/button').click()
    # 点击cis自动登录
    time.sleep(2)
    element_cis = get_element(chrome, (By.XPATH, '//*[@id="app"]/div/form/div[2]/div/div[4]/span'))
    element_cis.click()
    # 点击航线数据，进入下载界面
    element_skyline = get_element(chrome, (By.XPATH, '//*[@id="app"]/div/div[1]/div[2]/div[1]/div/ul/div[8]/div/li/img'))
